...
def twitter_get(select_hashtag_keyword, select_language, user_word_entry, num_of_tweets):  
    ...
    consumer_key = 'L3Lz91rRdyPuVhrMfpZduOIzl'
    consumer_secret = 'XZGSrnPn9lE92XyJFgIETu7AQr5K9AS0iFzofhdzO1GVhpl3iq'
    access_token = '1325114868798775298-F7ThBjWCD4pNm5YX1LD0rvugFBG3sy'
    access_token_secret = 'agviI49FjBb6akCLm3v8KzUPQsWSrYc68jqb3sqpwLKMY'
    ...
    # Scenario 2: Specific language (English or Danish)
    if select_language != 'All':
        tweets = tw.Cursor(api.search,
                            q=user_word,
                            tweet_mode = "extended",
...
def feature_extract(df):
    ...
    # Create stopword count features (english and Danish)
    df['stopword_en_ct'] = df.full_text.apply(lambda x: len([x for x in x.split() if x in stopwords_en]))
    df['stopword_da_ct'] = df.full_text.apply(lambda x: len([x for x in x.split() if x in stopwords_da]))
...
def text_clean_round3(text):
    ...
    stopwords_en = nltk.corpus.stopwords.words('english')
    stopwords_da = nltk.corpus.stopwords.words('danish')
    stopwords = stopwords_en + stopwords_da
    # Create pre-clean character count feature
...
#----------------
# Function to convert  
def word_cloud_all(df, wordcloud_words, query): 
    ...
    if len(queryList) > 1:
        for query in queryList:
            str2 = str2.replace(str(query), "")
    else:
        str2 = str2.replace(str(query), "")
    
    # generate word cloud
    wordcloud = WordCloud(max_words=100, background_color="black", collocations=False, mask = twitterMask, contour_width=3, contour_color='black').generate(str2)
    return wordcloud
...
twitterMask = np.array(Image.open("images/twitterMask.png"))
...
def default_wordcloud(text_sentiment):
    ...
    st.write('Word cloud with the most occuring words, where size equals frequency.')
    st.pyplot()
...
def plot_wordcloud(submitted2, score_type, text_sentiment, wordcloud_words, top_n_tweets, searchQuery):
    ...
    # Run wordlcloud for top n tweets
    if score_type == 'All':         
        wordcloud = word_cloud_all(text_sentiment, wordcloud_words, searchQuery)
    # else:
    #     wordcloud = word_cloud_sentiment(text_sentiment, score_type_nm, top_n_tweets, wordcloud_words)

    # Display the generated wordcloud image:
    st.set_option('deprecation.showPyplotGlobalUse', False)
    plt.imshow(wordcloud.recolor(color_func=WC_color, random_state=3), interpolation='bilinear')
    plt.axis("off")
    plt.show()
